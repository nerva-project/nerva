...
`--tx-proxy` which tells `nervad` the ip/port of a socks proxy provided by a
...
`nervad` currently selects two outgoing connections every 5 minutes for
...
`nervad` will have rotated its selected outgoing connections several times in
...
circuits is observable by the ISP. Instead, `nervad` should likely disconnect
